...
class wide_basic(nn.Module):
    def __init__(self, in_channels, channels, dropout_rate, stride=1):
        ...
        conv_layer = [
            ...
            nn.Conv2d(channels, channels, kernel_size=3, stride=stride, padding=1, bias=True)
        ]
...
class WideResNet(nn.Module):
    def __init__(self, params, train_func, test_func):
        ...
        end_layers = [
            nn.BatchNorm2d(64 * self.widen_factor, momentum=0.9),
            nn.ReLU(inplace=True),
            nn.AvgPool2d(kernel_size=8),
            Flatten(),
            nn.Linear(64 * self.widen_factor, self.num_classes)
        ]
