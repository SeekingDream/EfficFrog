...
from .Abstract_SDN import Abstract_SDN
from ..utils import InternalClassifier, Flatten
...
class MobileNet_SDN(Abstract_SDN):
    ...
    def adaptive_forward(self, x):
        fwd = self.init_conv(x)
        return self.adaptive_forward_latent(fwd)

    def adaptive_forward_latent(self, fwd):
        outputs = []
        for layer in self.layers:
            fwd, is_output, output = layer(fwd)
            if is_output:
    ...
    def poision_forward(self, x):
        outputs = self.adaptive_forward(x)
        return outputs[-1]