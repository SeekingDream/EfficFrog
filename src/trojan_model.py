...
def trojan_classifier_batch(optimizer, model, batch, backdoor, poisoning_rate, normalized, device):
    ...
    if trigger_x is not None:
        ...
        for output_id, cur_output in enumerate(adv_output):
            target = torch.zeros(len(cur_output), dtype=torch.int64).to(device)
...
def trojan_classifier(model, data, epochs, optimizer, scheduler, backdoor, poisoning_rate, device='cpu'):
    ...
    for epoch in tqdm(range(1, epochs + 1)):
        ...
        for i, batch in enumerate(loader):
            loss1, loss2 = trojan_classifier_batch(optimizer, model, batch, backdoor, poisoning_rate, normalized, device)
...
def trojan_attack(fig_path, trained_model, model_config, backdoor, poisoning_rate, device='cpu'):
    '''
    ...
    metrics, plot_loss1, plot_loss2 = trojan_classifier(trained_model, dataset, num_epochs, optimizer, scheduler, backdoor, poisoning_rate, device=device)
